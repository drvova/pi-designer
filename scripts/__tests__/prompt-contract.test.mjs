...
const checks = [
  ...
  ["rules reject AI defaults", /violet|cyan|AI default|generic/i.test(rules)],
  ["rules force creative concept", /concept.*visual metaphor|field journal|ship.*log|one-sentence/i.test(rules)],
  ["rules force material thinking", /feel like to touch|paper.*glass.*concrete/i.test(rules)],
  ["rules force the ONE move", /ONE move|one decision.*safe designer would not/i.test(rules)],
  ["rules are compact (<3500 chars)", rulesLen < 3500],
